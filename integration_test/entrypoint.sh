...
DEPLOYMENT_JSON=$(cat $MXC_MONO_DIR/packages/protocol/deployments/l1_test_L1.json)
L2_GENESIS_ALLOC=$(cat $DIR/nodes/deployments/mainnet.json)

MXC_L1_CONTRACT_ADDRESS=$(echo $DEPLOYMENT_JSON | jq .contracts.MXCL1 | sed 's/\"//g')
MXC_L2_CONTRACT_ADDRESS=$(echo $L2_GENESIS_ALLOC | jq 'to_entries[] | select(.value.contractName=="MXCL2") | .key' | sed 's/\"//g')
...
if [ "$RUN_TESTS" == "true" ]; then
    ...
    MXC_L1_ADDRESS=$MXC_L1_CONTRACT_ADDRESS \
    MXC_L2_ADDRESS=$MXC_L2_CONTRACT_ADDRESS \
